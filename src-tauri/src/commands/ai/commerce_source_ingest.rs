...
use super::ai_proxy::{build_client_for_target, enforce_resolved_host, ProxyError};
...
fn issue_receipt(
    ...
    Ok(CommerceSourceIngestReceipt {
...
pub async fn ai_ingest_competition_source_image(
    ...
    while let Some(chunk) = stream.next().await {
        let chunk =
            chunk.map_err(|_| ProxyError::Request("Commerce source image body failed".into()))?;
