...
//! the Vectorizer.AI direct API; the API Secret stays in the OS credential
//! vault and is injected inside Rust, never returned to JS.
...
impl From<KeyringError> for VectorizeError {
    fn from(error: KeyringError) -> Self {
        match error {
            KeyringError::NoEntry => Self::MissingSecret,
            _ => Self::Keychain,
        }
    }
}
...
fn key_status_inner(api_id: &str) -> Result<bool, VectorizeError> {
    ...
    match vectorizer_entry(&api_id)?.get_password() {
        ...
        Err(error) => Err(VectorizeError::from(error)),
    }
}
