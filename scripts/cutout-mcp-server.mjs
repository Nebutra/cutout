...
const SERVER_INFO = { name: 'cutout-headless', version: '0.1.1' }
let PROJECT_ROOT
let closeHeadlessRuntime
let executeControl
let executeGovernance
let capabilityStatus
let discoveryHandshake
let listSkills
let readSkill
let closeRegistryRuntime
let registryApplyInstall
let registryGet
let registryList
let registryPlanInstall
let registryReceipt
let workflowCompatibility
let workflowGet
let workflowList
...
export const MCP_TOOLS = [
  ...
  { name: 'cutout_registry_apply_install', description: 'Re-resolve and apply a previewed conflict-free install after a host-issued approval lease bound to its plan and project revision.', inputSchema: { type: 'object', additionalProperties: false, required: ['itemId','framework','planId','approvalLeaseId'], properties: { itemId: { type: 'string', minLength: 1, maxLength: 160 }, version: { type: 'string', maxLength: 80 }, framework: { type: 'string', minLength: 1, maxLength: 80 }, planId: { type: 'string', minLength: 1, maxLength: 160 }, approvalLeaseId: { type: 'string', minLength: 1, maxLength: 160 } } } },
...
async function callTool(params) {
  ...
  if (!PROJECT_ROOT && !['cutout_capabilities_status', 'cutout_skills_list', 'cutout_skill_read'].includes(params.name)) {
    const result = {
      ok: false,
      error: {
  ...
  switch (params.name) {
    ...
    case 'cutout_registry_apply_install': result = { ok: true, response: await registryApplyInstall(PROJECT_ROOT, input.itemId, input.framework, input.planId, input.approvalLeaseId, input.version) }; break
...
export function runMcpServer(options) {
  ...
  closeRegistryRuntime = options.closeRegistryRuntime ?? (async () => undefined)
  ...
  workflowCompatibility = options.workflowCompatibility
  workflowGet = options.workflowGet
  workflowList = options.workflowList
