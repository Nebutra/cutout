...
describe('release-note catalog', () => {
  it('loads the reviewed v0.1.22 entry with all five shipped locales and no historical backfill', async () => {
    const loaded = await loadReleaseNotesCatalog(undefined, { requireAllLocales: true })
    expect(loaded.entries.map((value) => value.version)).toEqual(['0.1.22', '0.1.20', '0.1.19'])
    expect(Object.keys(requireReleaseNotesEntry(loaded, '0.1.22').locales)).toEqual(RELEASE_NOTES_LOCALES)
  ...
  it('renders all release inputs through the CLI from one exact entry', async () => {
    ...
    const result = spawnSync(process.execPath, ['scripts/release-notes.mjs', 'render', '--version', '0.1.22', '--output', output, '--require-all-locales'], { cwd: process.cwd(), encoding: 'utf8' })
    ...
    expect(github).toContain('Cutout v0\\.1\\.22')
  })
})
