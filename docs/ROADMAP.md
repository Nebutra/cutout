...
| --- | --- | --- | --- | --- |
| Local Agent, Canvas, Deliver | Local projects, Design IR, preview, approval, export, CLI, MCP, Skills contracts, and a desktop-internal durable local Agent Host with native-routed Provider and bounded Coding backends exist. | Stable schemas, migrations, policy, accessibility, packaged lifecycle and release gates. | Supported local journeys pass unit, integration, crash/restart, sleep/wake, visual, privacy, migration, and packaged-app smoke tests; manifest matches behavior. | The desktop Host executes only assigned Providers and structured Coding patches under native approval; it exposes no arbitrary shell. The default CLI/MCP headless host has no bundled Provider or Coding executor. Do not claim live sync, cloud execution, web search, or video processing. |
...
| Release evidence | Local macOS builds plus release scripts exist. Tauri updater artifacts, stable/beta manifest generation, hash/SBOM/provenance metadata and release-only CI are implemented and fixture-tested; no production signing key or newly published manifest is recorded by source alone. | Reproducible build, protected CI secrets, HTTPS distribution, privacy manifests, clean-machine tests and owner review. | Release checklist completes with immutable artifacts, updater/platform signature verification, attestations and published signed forward-update metadata. | Local fixtures and CI definitions do not make a DMG distributed, signed, notarized or automatically updating. Staged rollout and automatic rollback are intentionally unsupported until a signed mutable control plane enforces them. |
...
| Hosted Codex plugin MCP | A repo marketplace, local Codex plugin package and project-bound stdio MCP runtime exist; no hosted Cutout project or OAuth MCP service exists. | Cloud project authority, OAuth client and consent, tenant isolation, encrypted storage, scoped authorization, rate/spend policy, privacy/legal review, observability and incident operations. | A public marketplace install authenticates against production, binds only authorized projects, preserves preview/approval/provenance, passes tenant-isolation and revoke/delete tests, and has rollout evidence. | Do not claim one-install hosted tools, account connectivity, remote projects, OAuth, or public marketplace availability from the local plugin package. |
| Remote media providers | Catalog definitions and protocol contracts exist; most remote media/speech execution is not bundled. | Approved accounts and terms, credential custody, moderation, regional policy, quotas, cancellation and provenance. | Authorized sandbox and failure tests pass; privacy owners approve; discovery reflects actual models. | Do not claim Deepgram, ElevenLabs, AssemblyAI, Replicate, fal, video, or remote media execution from catalog presence. |
...
| --- | --- | --- | --- |
| Apple Developer ID, notarization, updater | Updater artifact/manifest infrastructure and fixture tests exist, but source alone does not prove a distribution identity, notarization ticket, stapled artifact, protected updater key or published endpoint. | Active Apple membership; Developer ID identities; hardened entitlements; successful notarization/stapling; clean-Mac Gatekeeper validation. Updater also needs protected keys, published HTTPS manifests and real signature-failure evidence. | Do not claim signed distribution, notarization, Gatekeeper readiness, automatic updates, staged rollout or automatic rollback from local fixtures or workflow code. |
