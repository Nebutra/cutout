...
import { defaultPersonalizationSettings, personalizationSettingsSchema, type PersonalizationSettings } from './schema'
...
export function createPersonalizationService(store:PersonalizationStore=defaultStore()):PersonalizationService{return{
  async load(){try{const raw=await store.get<unknown>(STORE_KEY);return raw===undefined?defaultPersonalizationSettings:personalizationSettingsSchema.parse(raw)}catch{return defaultPersonalizationSettings}},
