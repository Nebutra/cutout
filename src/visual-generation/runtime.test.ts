...
describe('VisualTaskRuntime',()=>{
  it('runs the canonical plan and reuses its durable promotion after restart',async()=>{
    ...
    const first=createVisualTaskRuntime({tools:{invoke},reviewer:approveFirstVisualCandidate(),store:createStorageVisualExecutionStore(storage),append})
    ...
    const restarted=createVisualTaskRuntime({tools:{invoke},reviewer:approveFirstVisualCandidate(),store:createStorageVisualExecutionStore(storage),append})
  ...
  it('does not auto-promote a task that requires human review',async()=>{const runtime=createVisualTaskRuntime({tools:{invoke:toolResult},reviewer:approveFirstVisualCandidate('agent'),store:createStorageVisualExecutionStore(memoryStorage()),append:vi.fn()});await expect(runtime.execute('run',{...task,publication:{...task.publication,requiresHumanReview:true}})).rejects.toThrow('human review')})
})
