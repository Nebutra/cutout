...
export class IndexedDbLibraryBlobStore {
  ...
  async put(bytes:Uint8Array, mediaType:string):Promise<LibraryBlobRecord> {
    ...
    try { const existing=await req(db.transaction(STORE,'readonly').objectStore(STORE).get(sha256)) as LibraryBlobRecord|undefined; if(existing)return existing
      ...
      const at=this.now(),record={sha256,mediaType,size:bytes.byteLength,bytes:new Uint8Array(bytes),createdAt:at,lastAccessedAt:at}; const tx=db.transaction(STORE,'readwrite'),store=tx.objectStore(STORE);if(store.keyPath)store.add(record);else store.add(record,sha256);await done(tx);return record
    } finally { db.close() }
  }
...
function done(tx:IDBTransaction){return new Promise<void>((resolve,reject)=>{tx.oncomplete=()=>resolve();tx.onerror=()=>reject(tx.error);tx.onabort=()=>reject(tx.error??new Error('Blob transaction aborted.'))})}
