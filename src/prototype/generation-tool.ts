...
export const GENERATION_DECISION_MAX_OUTPUT_TOKENS = 2_000
export const GENERAL_TOOL_LOOP_MAX_OUTPUT_TOKENS = 6_000

const SHORT_CLASSIFICATION_TOOLS = new Set([
  'proceed_with_generation',
  'ask_clarifying_question',
  'reply_conversationally',
])

export function generationToolLoopMaxOutputTokens(
  toolNames: readonly string[],
): number {
  return toolNames.every((name) => SHORT_CLASSIFICATION_TOOLS.has(name))
...
> {
  return {
    ...
    description:
      ...
      + 'change the route topology. This tool only decides whether to proceed and may refine the brief; '
      + 'the Planner remains the sole owner of Design System directions, route topology, and materials.',
