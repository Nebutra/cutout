...
function buildChildren(spec: GraphSpec): ReadonlyMap<string, readonly string[]> {
 ...
 */
export function subtreeIds(spec: GraphSpec, nodeId: string): ReadonlySet<string> {
...
async function schedule<TOut>(
  ...
  const run = async (node: GraphNodeSpec): Promise<void> => {
    ...
    try {
      ...
      const output = await deps.runNode(node, inputs)
...
/**
 * Run a whole graph from scratch — every node starts `idle`. Never throws: a
 * node failure is captured as its `error` state; the resolved {@link RunResult}
 * carries the final state of every node.
 */
export function runGraph<TOut>(
  spec: GraphSpec,
  deps: RunDeps<TOut>,
): Promise<RunResult<TOut>> {
 ...
 */
export function reRunSubtree<TOut>(
  spec: GraphSpec,
  nodeId: string,
  deps: RunDeps<TOut>,
  prior: ReadonlyMap<string, NodeRunState<TOut>>,
): Promise<RunResult<TOut>> {
