...
- [x] Require maturity adapters to asynchronously re-verify complete retained
      evidence; reject synchronous projection of precomputed reports.
...
- [x] Add structural protected-surface audit plus a behavioral held-out contract
      Profile for conformance only.
- [ ] Add promotion-packet validation backed by retained evidence from two
      distinct real Profiles; deterministic cross-Host fixtures remain contract
      tests only.
  - The trusted async verifier registry, strict verified-proof envelope and stale
    ChangeSet handoff guard are implemented. Completion remains blocked on two
    independently accepted real retained Profile bundles; contract tests cannot
    satisfy this item.

## Gate 4: Reference Adapters

- [x] Wrap Commerce as the first real manifest and prove canonical graph/Plan
      parity; register its full receipt/byte rehearsal verifier without claiming
      maturity from deterministic test data.
- [x] Add the held-out contract Profile without protected-surface changes.
...
- [x] Prove node-scoped repair, sibling-hash retention and exact identity-lock
      impact in Commerce plus Game Asset/held-out contract tests.
...
- [x] Run Profile schema/closure/registry/composition/lifecycle/conformance and
      Project Bundle suites after the no-mock rework.
