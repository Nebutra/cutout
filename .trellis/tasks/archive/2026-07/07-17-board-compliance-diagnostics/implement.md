...
- [x] 1. Recon: read `src/prototype/region-deconstruct.ts` (current working-tree
...
- [x] 2. Add `BOARD_BORDER_WHITE_MIN_RATIO = 0.55` to `src/algorithm/constants.ts`
      (documented as new/tunable, outside the verbatim-port contract).
- [x] 3. Create `src/algorithm/boardDiagnostics.ts` per design (single pass,
      reuse `isBackgroundPixel`, band floor 2px, no frame mutation).
- [x] 4. Create `src/algorithm/boardDiagnostics.test.ts` (cases in design).
- [x] 5. Wire into `region-deconstruct.ts`: diagnostics computed from the
      ...
      module doc comment.
- [x] 6. Update `region-deconstruct.test.ts` stubs + add assertions (callback
      order, result record, no behavior change when compliant).
- [x] 7. `regionBoardPrompt`: add the no-text-labels sentence; update/add the
      prompt unit test.
- [x] 8. `IntentWorkspace.tsx`: pass `onRegionDiagnostics` logging non-compliant
...
- [x] 9. Gates:
      ...
      - `npx oxlint src`
- [x] 10. Confirm no unrelated files touched (working tree has other in-flight
