...
test("Updates stays truthful and configurable without a desktop runtime", async ({ page }) => {
  ...
  await expect(updates.getByText("No release notes are available for this version.")).toBeVisible();
  await expect(updates.getByRole("button", { name: "Open" })).toBeDisabled();
